...
ifdef simplesamlphp
	...
	@curl -sSL https://github.com/simplesamlphp/simplesamlphp/releases/download/v1.17.7/simplesamlphp-1.17.7.tar.gz | tar  --transform 's/simplesamlphp-[0-9]+\.[0-9]+\.[0-9]+/simplesamlphp/x1' -C htdocs/auth/saml/extlib -xzf - # SimpleSAMLPHP release tarball already has all composer dependencies.
...
mergebase := $(shell git fetch gerrit >/dev/null 2>&1 && git merge-base HEAD gerrit/19.04_STABLE)
...
push: securitycheck minaccept
	...
	@if test -z "$(TAG)"; then \
		git push gerrit HEAD:refs/publish/19.04_STABLE; \
	else \
		git push gerrit HEAD:refs/publish/19.04_STABLE/$(TAG); \
	fi

security: minaccept
	@echo "Pushing the SECURITY change upstream..."
	@if test -z "$(TAG)"; then \
		git push gerrit HEAD:refs/drafts/19.04_STABLE; \
	else \
		git push gerrit HEAD:refs/drafts/19.04_STABLE/$(TAG); \
	fi
