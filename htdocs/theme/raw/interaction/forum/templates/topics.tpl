...
{/if}
    ...
    {if $membership && (!$forum->subscribed || $moderator)}
        <input id="topic_{$topic->id}" type="checkbox" name="checked[{$topic->id}]" class="topic-checkbox">
    {/if}
    </td>
    <td class="topic">
        <h3 class="title"><label for="topic_{$topic->id}"><a href="{$WWWROOT}interaction/forum/topic.php?id={$topic->id}">{$topic->subject}</a></label>{if $publicgroup}<a href="{$topic->feedlink}"><img class="feedicon" src="{theme_url filename='images/feed.png'}"></a>{/if}</h3>
