    ...
    <div class="authInstance" id="instanceDiv{{$instance->id}}">{{strip}}
      <span class="authitem{{if $instance->active == 0}} inactive{{/if}}">
        ...
        <span class="authIcons" id="arrows{{$instance->id}}">
            ...
            <a class="btn btn-link text-midtone" href="" onclick="PieformAuthlist.move_down({{$instance->id}}); return false;">
            ...
            {{/if}}
            {{if $instance->index != 0 }}
            <a class="btn btn-link text-midtone" href="" onclick="PieformAuthlist.move_up({{$instance->id}}); return false;">
                <span class="icon icon-long-arrow-alt-up" role="presentation" aria-hidden="true"></span>
                <span class="sr-only">{{str tag=moveitemup}}</span>
            </a>
            {{/if}}
            <a href="" class="btn btn-default btn-sm" onclick="PieformAuthlist.remove_auth({{$instance->id}}); return false;">
...
<div class="select">
    ...
    <button id="authlistDummyButton" class="btn btn-primary" type="button" onclick="PieformAuthlist.create_auth(); return false;" name="button" value="foo">{{str tag=Add section=admin}}</button>
</div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" data-height=".modal-body">
            <div class="modal-header">
                ...
                <h4 class="modal-title authinstance-header text-inline"></h4>
