<div class="bt-{$blocktype}-editor js-blockinstance blockinstance gridstackblock card card-secondary clearfix {if $configure} configure{elseif $retractable} collapsible{/if}" data-id="{$id}" id="blockinstance_{$id}{if $configure}_configure{/if}">
    <h3 class="card-header js-heading drag-handle {if !$title}card-header-placeholder{/if} access-drop-handle" title="{$strmovetitletexttooltip}">
        ...
        <span class="blockinstance-header{if $retractable && $retractedonload} retracted{/if}">
            {if $configure}{$configtitle}: {str tag=Configure section=view}{else}{$title|default:"[$strnotitle]"}
            {/if}
        </span>
    ...
    <div class="block blockinstance-content js-blockinstance-content{if $retractable} collapse{if $retractedonload} {else} show{/if}{/if}" id="blockinstance_{$id}_target" {if $loadbyajax}data-blocktype-ajax="{$id}"{else}data-blocktype-noajax="{$id}"{/if}>
        {$content|safe}
    </div>
</div>
