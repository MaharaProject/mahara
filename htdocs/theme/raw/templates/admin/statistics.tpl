...
{if $sitedata}
    <div id="site-stats-wrap" class="panel-items js-masonry" data-masonry-options='{ "itemSelector": ".panel" }'>
        <div class="panel panel-info">
            <h3 class="panel-heading">{$sitedata.name}: {str tag=siteinformation section=admin} <span class="icon icon-info pull-right" role="presentation"></span></h3>
        ...
        <div class="panel panel-default double">
            ...
            {if $subpagedata.table.csv}
                <a href="{$WWWROOT}download.php" class="panel-footer" target="_blank"><span class="icon icon-table" role="presentation"></span> {str tag=exportstatsascsv section=admin}</a>
            {/if}
        </div>
