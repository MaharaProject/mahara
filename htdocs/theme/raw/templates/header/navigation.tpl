...
{foreach from=$item.submenu item=subitem}
                        <li class="{if $subitem.selected}selected {/if}{if $subitem.submenu}has-sub {/if}"><span>
                            <a href="{if $item.wwwroot}{$item.wwwroot}{else}{$WWWROOT}{/if}{$subitem.url}"{if $subitem.accesskey} accesskey="{$subitem.accesskey}"{/if}>{$subitem.title}</a>
                        </span>
                        {if $subitem.submenu}
                            <ul class="dropdown-tertiary">
                                {foreach from=$subitem.submenu item=tertiaryitem}
                                    <li{if $tertiaryitem.selected} class="selected"{/if}><span>
                                        <a href="{if $item.wwwroot}{$item.wwwroot}{else}{$WWWROOT}{/if}{$tertiaryitem.url}"{if $tertiaryitem.accesskey} accesskey="{$tertiaryitem.accesskey}"{/if}>{$tertiaryitem.title}</a>
                                    </span></li>
                                {/foreach}
                            </ul>
                        {/if}
...
{foreach from=$SELECTEDSUBNAV item=item}
                <li{if $item.selected} class="selected"{/if}><span><a href="{if $item.wwwroot}{$item.wwwroot}{else}{$WWWROOT}{/if}{$item.url}"{if $item.accesskey} accesskey="{$item.accesskey}"{/if}>{$item.title}</a></span></li>
