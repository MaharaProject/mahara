...
{if $group->membershiptype == 'member'}
    ...
    <li class="leavegroup">
    {if $group->canleave}
      <a href ="{$WWWROOT}group/leave.php?id={$group->id}&amp;returnto={$returnto}" class="btn-leavegroup"><span class="btn-leavegroup">{str tag="leavegroup" section="group"}</span></a>
    {/if}
...
{elseif $group->membershiptype == 'admin'}
	<li class="admincontrol"><a href="{$WWWROOT}group/edit.php?id={$group->id}" title="{str tag=edit}" class="btn-editgroup"><span class="btn-edit">{str tag=edit}</span></a>
	<a href="{$WWWROOT}group/delete.php?id={$group->id}" title="{str tag=delete}" class="btn-deletegroup"><span class="btn-del">{str tag=delete}</span></a></li>
	
{if $group->requests}
	<li class="requestspending">
		<a href="{$WWWROOT}group/members.php?id={$group->id}&amp;membershiptype=request" class="btn-membershiprequests"><span class="btn-pending">{str tag="membershiprequests" section="group"} ({$group->requests})</span></a>
	</li>
{/if}
...
{elseif $group->request}
	<li class="requesttojoin"><a href="{$WWWROOT}group/requestjoin.php?id={$group->id}&amp;returnto={$returnto}" class="btn-request"><span class="btn-request">{str tag="requestjoingroup" section="group"}</span></a></li>
