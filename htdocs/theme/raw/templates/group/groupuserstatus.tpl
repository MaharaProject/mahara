...
<div class="btn-action-list">
    <div class="btn-top-right btn-group btn-group-top">
        {if $list}
        <a href="#" class="label-btn btn btn-secondary btn-sm" data-id="{$group->id}" title='{str tag="labelfor" section="group" arg1=$group->name}'>
            <span class="icon icon-tag" role="presentation" aria-hidden="true"></span>
            <span class="btn-title">{str tag="label" section="group"}</span>
        </a>
        {/if}
        {if $group->canleave}
            <a href ="{$WWWROOT}group/leave.php?id={$group->id}" class="btn btn-secondary btn-sm">
        ...
        {if $group->invitefriends}
            <a href ="{$WWWROOT}group/inviteusers.php?id={$group->id}&friends=1" class="btn btn-secondary btn-sm">
        ...
        {elseif $group->suggestfriends && ($group->request || $group->jointype == 'open')}
            <a href ="{$WWWROOT}group/suggest.php?id={$group->id}" class="btn btn-secondary btn-sm">
...
{elseif $group->membershiptype == 'admin'}
    <div class="btn-action-list">
        <div class="btn-top-right btn-group btn-group-top">
            ...
            {/if}
            {if $viewid}
            <a href="{$WWWROOT}view/blocks.php?id={$viewid}&group={$group->id}" title="{str tag=editcontentandlayout section='view'}" class="btn btn-secondary btn-sm">
            ...
            <a href="{$WWWROOT}group/edit.php?id={$group->id}" title="{str(tag=editspecific arg1=$group->name)|escape:html|safe} {str tag=settings}" class="btn btn-secondary btn-sm">
            ...
            {if $cancopy}
            <a href="{$WWWROOT}group/copy.php?id={$group->id}&return=mygroups" title="{str(tag=copygroup section=group arg1=$group->name)|escape:html|safe}" class="btn btn-secondary btn-sm">
            ...
            {/if}
            <a href="{$WWWROOT}group/delete.php?id={$group->id}" title="{str(tag=deletespecific arg1=$group->name)|escape:html|safe}" class="btn btn-secondary btn-sm">
