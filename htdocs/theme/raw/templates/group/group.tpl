...
{if $group->editwindow}
<div>{str tag=editable section=group}: {$group->editwindow}</div>
{/if}
{if $group->membercount}
<div class="inline s"><a href="{$WWWROOT}group/members.php?id={$group->id}">{$group->membercount} {if $group->membercount > 1}{str tag=members section=group}{else}{str tag=member section=group}{/if}</a></div>
{/if}
