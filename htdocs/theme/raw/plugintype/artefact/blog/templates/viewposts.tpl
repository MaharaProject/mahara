    ...
    <div class="list-group-item clearfix flush">
        <div class="post-heading">
            <h3 class="list-group-item-heading title">
                {if !$options.editing}
                    <a class="modal_link" data-bs-toggle="modal-docked" data-bs-target="#configureblock" href="#" data-blockid="{$options.blockid}" data-artefactid="{$post->id}">
                        {$post->title}
                    </a>
                {else}
                    {$post->title}
            ...
            <div class="collapse" id="post-attach-{$post->id}">
                ...
                {foreach from=$post->files item=file}
                    <li class="list-group-item">
                        <a class="modal_link file-icon-link" data-bs-toggle="modal-docked" data-bs-target="#configureblock" href="#" data-blockid="{$options.blockid}" data-artefactid="{$file->attachment}">
                        {if $file->icon}
                        ...
                        <span class="title">
                        {if !$options.editing}
                            <a class="modal_link" data-bs-toggle="modal-docked" data-bs-target="#configureblock" href="#" data-blockid="{$options.blockid}" data-artefactid="{$file->attachment}">
                        {/if}
