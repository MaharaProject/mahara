...
<div id="universalsearch-filter-owner" class="search-filter-owner pieform with-label-widthauto form-condensed">
    <input type="hidden" id="search-filter-owner-url" value="{$WWWROOT}{$thispath}?query={$query}&mainfacetterm={$selected}&secfacetterm={$contentfilterselected}&license={$license}&sort={$sort}{if $tagsonly}&tagsonly=true{/if}&limit={$limit}" />
    <div class="select form-group">
        <label for="search-filter-owner">
            {if $selected eq 'Text' || $selected eq 'Media' || $selected eq 'Portfolio'}
                {str tag=owner section=search.elasticsearch7}:
            {else}
                {str tag=admin section=search.elasticsearch7}:
            {/if}
        </label>
        <span class="picker">
