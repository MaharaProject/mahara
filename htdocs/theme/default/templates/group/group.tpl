...
{if $group->canleave}
    <li><a href = "{$WWWROOT}group/leave.php?id={$group->id|escape}&amp;returnto={$returnto}">{str tag="leavegroup" section="group"}</a></li>
{/if}
...
{if $group->jointype == 'request' && $group->requests}
    <li>
    {if $group->requests == 1}
        <a href="{$WWWROOT}group/view.php?id={$group->id|escape}&amp;pending={$group->requests|escape}#members">{str tag="thereispendingrequest" section="group"}</a>
    {else}
...
<li><a href="{$WWWROOT}group/requestjoin.php?id={$group->id|escape}&amp;returnto={$returnto}">{str tag="requestjoingroup" section="group"}</a></li>
