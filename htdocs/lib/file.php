 ...
 */
function send_file($path, $filename, $lifetime=86400 , $pathisstring=false, $forcedownload=true, $mimetype='') {

    // Stuff from moodle $CFG; need to work out what to do about it.
    $disablebyteserving = false;
    $wwwroot = get_config('wwwroot');
    $usesid = false;
    $sessioncookie = '';

    // Use given MIME type if specified, otherwise guess it using
    // mimeinfo.  IE, Konqueror and Opera open html file directly in
    // browser from web even when directed to save it to disk :-O only
    // Firefox saves all files locally before opening when
    // content-disposition: attachment stated

    // Mahara has no check_browser_version function
    ...
    if ($lifetime > 0) {
        ...
        if (empty($disablebyteserving) && !$pathisstring && $mimetype != 'text/plain' && $mimetype != 'text/html') {
        ...
        if (strpos($wwwroot, 'https://') === 0) { //https sites - watch out for IE! KB812935 and KB316431
    ...
    if ($mimetype == 'text/html' && !empty($usesid) && empty($_COOKIE['MoodleSession'.$sessioncookie])) {
