...
function record_exists($table, $field1=null, $value1=null, $field2=null, $value2=null, $field3=null, $value3=null) {
    ...
    return record_exists_sql('SELECT * FROM '. $CFG->dbprefix . $table .' '. $select,$values);
}
...
function count_records($table, $field1=null, $value1=null, $field2=null, $value2=null, $field3=null, $value3=null) {
    ...
    return count_records_sql('SELECT COUNT(*) FROM '. $CFG->dbprefix . $table .' '. $select, $values);
}
...
function count_records_select($table, $select='', $values=null, $countitem='COUNT(*)') {
    ...
    return count_records_sql('SELECT '. $countitem .' FROM '. $CFG->dbprefix . $table .' '. $select, $values);
}
...
function get_record($table, $field1, $value1, $field2=null, $value2=null, $field3=null, $value3=null, $fields='*') {
    ...
    return get_record_sql('SELECT '.$fields.' FROM '. $CFG->dbprefix . $table .' '. $select, $values);
}
...
/// Default situation
    ...
    /*
    if (debugging()) {  // @todo nigel change this
...
function get_recordset_select($table, $select='', $values=null, $sort='', $fields='*', $limitfrom='', $limitnum='') {
    ...
    return get_recordset_sql('SELECT '. $fields .' FROM '. $CFG->dbprefix . $table . $select . $sort .' '. $limit, $values);
}
...
function get_field($table, $return, $field1, $value1, $field2=null, $value2=null, $field3=null, $value3=null) {
    ...
    return get_field_sql('SELECT ' . $return . ' FROM ' . $CFG->dbprefix . $table . ' ' . $select, $values);
}
...
function set_field($table, $newfield, $newvalue, $field1, $value1, $field2=null, $value2=null, $field3=null, $value3=null) {
    ...
    $stmt = $db->Prepare('UPDATE '. $CFG->dbprefix . $table .' SET '. $newfield  .' = ? '. $select);
...
function delete_records($table, $field1=null, $value1=null, $field2=null, $value2=null, $field3=null, $value3=null) {
    ...
    $stmt = $db->Prepare('DELETE FROM '. $CFG->dbprefix . $table .' '. $select);
    return $db->Execute($stmt,$values);
}
...
function delete_records_select($table, $select='',$values=null) {
    ...
    if (!empty($values) && is_array($values) && count($values) > 0) {
        ...
        $result = $db->Execute('DELETE FROM '. $CFG->dbprefix . $table .' '. $select);
    }
    return $result;
}
...
function insert_record($table, $dataobject, $primarykey=false, $returnpk=false) {
    ...
    if (is_array($table_columns) && array_key_exists($table,$table_columns)) {
        ...
        if (!$columns = $db->MetaColumns($CFG->dbprefix . $table)) {
            return false;
        }
    ...
    if (!empty($primarykey)) {
        ...
        if (!empty($returnpk)) {
            $pksql = "SELECT NEXTVAL('{$CFG->dbprefix}{$table}_{$primarykey}_seq')";
  ...
  // Pull out data matching these fields
    ...
    $numddd = count($ddd);
    $count = 0;
    $insertSQL = 'INSERT INTO '.$CFG->dbprefix . $table .' (';
    $fields = '';
    $values = '';
...
/// to find the sequence.
    ...
    if (is_postgres()) {
        ...
        $oidsql = 'SELECT '. $primarykey .' FROM '. $CFG->dbprefix . $table .' WHERE oid = '. $id;
        if ( ($rs = $db->Execute($oidsql))
             && ($rs->RecordCount() == 1) ) {
...
function update_record($table, $dataobject, $where=null) {
    ...
    $stmt = $db->Prepare('UPDATE '. $CFG->dbprefix . $table .' SET '. $update .' WHERE '.$whereclause);
    if ($rs = $db->Execute($stmt,array_merge($values,$wherevalues))) {
        return true;
    } else {
        debugging($db->ErrorMsg() .'<br /><br />UPDATE '. $CFG->dbprefix . $table .' SET '. $update .' WHERE id = \''. $dataobject->id .'\'');
        if (!empty($CFG->dblogerror)) {
            $debug=array_shift(debug_backtrace());
            error_log("SQL ".$db->ErrorMsg()." in {$debug['file']} on line {$debug['line']}. STATEMENT:  UPDATE $CFG->dbprefix$table SET $update WHERE id = '$dataobject->id'");
        }
        return false;
    }
}
...
function column_type($table, $column) {
    ...
    if(!$rs = $db->Execute('SELECT '.$column.' FROM '.$CFG->dbprefix.$table.' WHERE 1=2')) {
        return false;
    }
...
function  is_postgres() {
    global $CFG;
    return (strpos($CFG->dbtype,'postgres' === 0));
}
