...
tinymce.PluginManager.add('maharaimage', function(editor) {
	...
	function showDialog(imageList) {
		...
		function attachedImageList() {
			...
			if (!editor.settings.image_filebrowser) {
				return images;
			}
			var attachments = window[editor.settings.image_filebrowser].selecteddata;
			var i = 1;
			for (var a in attachments) {
				if (attachments[a].artefacttype == 'image' || attachments[a].artefacttype == 'profileicon') {
					images.push({
						'text': attachments[a].title,
						'value': i++,
						'id': attachments[a].id,
		...
		function attachmentSelectedCallback(imageList) {
			return function(e) {
				...
				if (value > 0 && imageList[value]) {
					...
					srcCtrl.value(config.wwwroot + 'artefact/file/download.php?file=' + imageList[value].id);
					srcCtrl.fire('change');
					altCtrl.value(imageList[value].description);
				}
				changingAttachment = false;
			};
		}
		...
		function srcChange() {
			...
			var attachmentCtrl = win.find('#attachment');
			if (attachmentCtrl && !changingAttachment) {
				attachmentCtrl.value(0);
			}
		...
		if (imgElm.nodeName == 'IMG' && !imgElm.getAttribute('data-mce-object') && !imgElm.getAttribute('data-mce-placeholder')) {
			data = {
				...
				attachment: dom.getAttrib(imgElm, 'data-attachment'),
				width: width,
				height: height
			};
		} else {
		...
		var imageList = attachedImageList();
		var generalFormItems = [
			{
				name: 'attachment',
				type: 'listbox',
				label: get_string('attachedimage'),
				autofocus: true,
				values: imageList,
				onselect: attachmentSelectedCallback(imageList)
			},
