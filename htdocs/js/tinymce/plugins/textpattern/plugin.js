...
var textpattern = (function () {
  ...
  var $_fa8uexrtjkvrio5e = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_b4ohtes1jkvrio5q.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_b4ohtes1jkvrio5q.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_b4ohtes1jkvrio5q.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_23suoaryjkvrio5l.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_23suoaryjkvrio5l.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_23suoaryjkvrio5l.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_douojvrxjkvrio5i.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_douojvrxjkvrio5i.checkKeyCode(keyCodes, e)) {
        $_douojvrxjkvrio5i.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_douojvrxjkvrio5i.checkCharCode(charCodes, e)) {
  ...
  var $_davfnjrujkvrio5g = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_fa8uexrtjkvrio5e.getPatterns(editor.settings));
    $_davfnjrujkvrio5g.setup(editor, patternsState);
    return $_1331u0rsjkvrio5e.get(patternsState);
  });
  function Plugin () {
  }
