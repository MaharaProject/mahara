...
var codesample = (function () {
  ...
  var getSelectedCodeSample = function (editor) {
    ...
    if ($_14vwfqagjkvrikm9.isCodeSample(node)) {
      return node;
    }
    return null;
  };
  ...
  var getLanguages$1 = function (editor) {
    ...
    var customLanguages = $_btp6c7abjkvrikkn.getLanguages(editor);
    return customLanguages ? customLanguages : defaultLanguages;
  };
  var getCurrentLanguage = function (editor) {
    var matches;
    var node = $_6gsgheadjkvrikkp.getSelectedCodeSample(editor);
    if (node) {
  ...
  var $_g1s109ahjkvrikma = {
    getLanguages: getLanguages$1,
    getCurrentLanguage: getCurrentLanguage
  };

  var $_g65ax7aajkvrikkl = {
    open: function (editor) {
      var minWidth = $_btp6c7abjkvrikkn.getDialogMinWidth(editor);
      var minHeight = $_btp6c7abjkvrikkn.getDialogMinHeight(editor);
      var currentLanguage = $_g1s109ahjkvrikma.getCurrentLanguage(editor);
      var currentLanguages = $_g1s109ahjkvrikma.getLanguages(editor);
      var currentCode = $_6gsgheadjkvrikkp.getCurrentCode(editor);
      editor.windowManager.open({
        ...
        onSubmit: function (e) {
          $_6gsgheadjkvrikkp.insertCodeSample(editor, e.data.language, e.data.code);
        }
      });
    }
  };
  ...
  var register = function (editor) {
    editor.addCommand('codesample', function () {
      var node = editor.selection.getNode();
      if (editor.selection.isCollapsed() || $_14vwfqagjkvrikm9.isCodeSample(node)) {
        $_g65ax7aajkvrikkl.open(editor);
      } else {
        editor.formatter.toggle('code');
      }
    });
  };
  var $_cfgigia9jkvrikkk = { register: register };

  var setup = function (editor) {
    var $ = editor.$;
    editor.on('PreProcess', function (e) {
      $('pre[contenteditable=false]', e.node).filter($_14vwfqagjkvrikm9.trimArg($_14vwfqagjkvrikm9.isCodeSample)).each(function (idx, elm) {
    ...
    editor.on('SetContent', function () {
      var unprocessedCodeSamples = $('pre').filter($_14vwfqagjkvrikm9.trimArg($_14vwfqagjkvrikm9.isCodeSample)).filter(function (idx, elm) {
  ...
  var $_73ehqnaijkvrikmc = { setup: setup };

  var loadCss = function (editor, pluginUrl, addedInlineCss, addedCss) {
    var linkElm;
    var contentCss = $_btp6c7abjkvrikkn.getContentCss(editor);
  ...
  var $_bcjpigajjkvrikme = { loadCss: loadCss };
  ...
  var $_egb29dakjkvrikmf = { register: register$1 };

  var addedInlineCss = Cell(false);
  global.add('codesample', function (editor, pluginUrl) {
    var addedCss = Cell(false);
    $_73ehqnaijkvrikmc.setup(editor);
    $_egb29dakjkvrikmf.register(editor);
    $_cfgigia9jkvrikkk.register(editor);
    editor.on('init', function () {
      $_bcjpigajjkvrikme.loadCss(editor, pluginUrl, addedInlineCss, addedCss);
    });
    editor.on('dblclick', function (ev) {
      if ($_14vwfqagjkvrikm9.isCodeSample(ev.target)) {
        $_g65ax7aajkvrikkl.open(editor);
      }
    });
  });
