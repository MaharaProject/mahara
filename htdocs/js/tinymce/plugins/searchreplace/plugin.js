...
var searchreplace = (function () {
  ...
  var $_5gc2ysk1jkvrilyj = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_5gc2ysk1jkvrilyj.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_dddrehjzjkvrilyc.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_dddrehjzjkvrilyc.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_dddrehjzjkvrilyc.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_dddrehjzjkvrilyc.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_dddrehjzjkvrilyc.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_dddrehjzjkvrilyc.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_dddrehjzjkvrilyc.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_dddrehjzjkvrilyc.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_dddrehjzjkvrilyc.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_dddrehjzjkvrilyc.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_dddrehjzjkvrilyc.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_dddrehjzjkvrilyc.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_dddrehjzjkvrilyc.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_dddrehjzjkvrilyc.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_dddrehjzjkvrilyc.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_brdr1ak3jkvrilyq = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_brdr1ak3jkvrilyq.open(editor, currentIndexState);
    });
  };
  var $_56qm29k2jkvrilyn = { register: register };
  ...
  var $_alwqick4jkvrilyt = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_56qm29k2jkvrilyn.register(editor, currentIndexState);
    $_alwqick4jkvrilyt.register(editor, currentIndexState);
    return $_bcc0xrjyjkvrilya.get(editor, currentIndexState);
  });
  function Plugin () {
  }
