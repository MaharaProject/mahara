...
if (!empty($results['data'])) {
    if ($search->archivedsubmissions) {
        ...
        $USER->set_download_file(generate_csv($results['data'], $csvfields), 'archivedsubmissions.csv', 'text/csv', true);
    }
    else {
        $csvfields = array('username', 'email', 'firstname', 'lastname', 'preferredname', 'submittedto', 'specialid', 'submittedtime');
        $USER->set_download_file(generate_csv($results['data'], $csvfields), 'currentsubmissions.csv', 'text/csv', true);
    }
